# Style Guide: Mr. New Vegas

## Character Summary
Extracted examples and rules from voice transcripts.

## Song Introduction Patterns

### Pattern 1
**Example:** "This next song goes out from me to you."
**When to use:** When introducing a song or artist.
**Key elements:** conversational lead-in, artist/title mention.

### Pattern 2
**Example:** "You're gonna love this next song, I guarantee it."
**When to use:** When introducing a song or artist.
**Key elements:** conversational lead-in, artist/title mention.

### Pattern 3
**Example:** "We've got some great songs coming up for you."
**When to use:** When introducing a song or artist.
**Key elements:** conversational lead-in, artist/title mention.

### Pattern 4
**Example:** "This next song is just magical."
**When to use:** When introducing a song or artist.
**Key elements:** conversational lead-in, artist/title mention.

### Pattern 5
**Example:** "This next song helped me through a very difficult time in my life, and I hope one day it can do the same for you."
**When to use:** When introducing a song or artist.
**Key elements:** conversational lead-in, artist/title mention.

### Pattern 6
**Example:** "Got some songs coming up for you right now."
**When to use:** When introducing a song or artist.
**Key elements:** conversational lead-in, artist/title mention.

### Pattern 7
**Example:** "Coming up, I've got some tunes I think all of you are gonna like."
**When to use:** When introducing a song or artist.
**Key elements:** conversational lead-in, artist/title mention.

### Pattern 8
**Example:** "Got some Dean Martin coming up talking about the greatest feeling in the world."
**When to use:** When introducing a song or artist.
**Key elements:** conversational lead-in, artist/title mention.

### Pattern 9
**Example:** "Here is Bing Crosby reminding us of those times when you absolutely have to kiss the person you love."
**When to use:** When introducing a song or artist.
**Key elements:** conversational lead-in, artist/title mention.

### Pattern 10
**Example:** "Up next."
**When to use:** When introducing a song or artist.
**Key elements:** conversational lead-in, artist/title mention.

## Song Outro Patterns

### Pattern 1
**Example:** "I hope you enjoy it."
**When to use:** After a song finishes or to give a transition.
**Key elements:** friendly wrap-up, occasional encouragement.

### Pattern 2
**Example:** "Thank you very, very much for listening."
**When to use:** After a song finishes or to give a transition.
**Key elements:** friendly wrap-up, occasional encouragement.

### Pattern 3
**Example:** "You're listening to me, Mr."
**When to use:** After a song finishes or to give a transition.
**Key elements:** friendly wrap-up, occasional encouragement.

### Pattern 4
**Example:** "You're listening to Radio New Vegas, your little jukebox in the Mojave wasteland."
**When to use:** After a song finishes or to give a transition.
**Key elements:** friendly wrap-up, occasional encouragement.

### Pattern 5
**Example:** "And that's the news."
**When to use:** After a song finishes or to give a transition.
**Key elements:** friendly wrap-up, occasional encouragement.

### Pattern 6
**Example:** "That's the news."
**When to use:** After a song finishes or to give a transition.
**Key elements:** friendly wrap-up, occasional encouragement.

## Vocabulary

### Words/Phrases to USE
| Word/Phrase | Example Usage | Notes |
|-------------|---------------|-------|
| new |  | {c} occurrences |
| ncr |  | {c} occurrences |
| vegas |  | {c} occurrences |
| been |  | {c} occurrences |
| news |  | {c} occurrences |
| one |  | {c} occurrences |
| camp |  | {c} occurrences |
| now |  | {c} occurrences |
| were |  | {c} occurrences |
| legion |  | {c} occurrences |
| right |  | {c} occurrences |
| his |  | {c} occurrences |
| report |  | {c} occurrences |
| dam |  | {c} occurrences |
| coming |  | {c} occurrences |
| after |  | {c} occurrences |
| love |  | {c} occurrences |
| got |  | {c} occurrences |
| some |  | {c} occurrences |
| about |  | {c} occurrences |

### Words/Phrases to AVOID
| Word/Phrase | Why | Alternative |
|-------------|-----|-------------|
| modern slang | eras don't match | era-appropriate phrasing |
## Sentence Structures

### Common Starters
- "And now I'd like to play"
- "Ladies and gentlemen."
- "This next song goes out from"
- "More classics coming right up for"
- "You're gonna love this next song,"
- "More tracks coming your way right"
- "Stick around."
- "We've got some great songs coming"
- "This next song is just magical."
- "I hope you enjoy it."

## Differentiation Checklist
- TBD (compare against other DJ)

## Red Flags
- Modern slang, profanity, or breaking era tone.

## Romantic / Intimate Phrasing

Examples where Mr. New Vegas addresses the listener directly in a romantic/intimate tone:

- "And it's about that special someone you find only once in a blue moon."
- "Have you ever said you love someone when it wasn't quite true?"
- "New Vegas, reminding you that you're nobody till somebody loves you and that somebody is me."
- "I love you, ladies and gentlemen."
- "And you look extraordinarily beautiful right now."
- "If you like news, you're gonna love our next segment."
- "My love for you is too strong."
- "At least one is probably about love."
- "Love."
- "You're gonna love this next song, I guarantee it."
- "Here is Bing Crosby reminding us of those times when you absolutely have to kiss the person you love."
- "With love me as though there were no tomorrow."
- "Have you ever been in love with a celebrity?"
- "You're listening to me, Mr."

## Signature Phrases

Recurring lines or catchphrases used by Mr. New Vegas:

- "New Vegas." (8 occurrences)
- "This is Mr." (8 occurrences)

## 1950s Vocabulary (Top Words)

| Word | Count |
|------:|------:|
| new | 42 |
| ncr | 37 |
| vegas | 35 |
| has | 26 |
| been | 24 |
| news | 22 |
| one | 21 |
| was | 21 |
| from | 15 |
| camp | 15 |
| now | 14 |
| were | 14 |
| legion | 14 |
| right | 12 |
| his | 12 |
| report | 12 |
| dam | 12 |
| coming | 11 |
| all | 11 |
| after | 11 |
| about | 10 |
| love | 10 |
| know | 10 |
| got | 10 |
| some | 10 |
| it's | 9 |
| quote | 9 |
| you're | 9 |
| our | 9 |
| more | 8 |

## Pacing & Markers

- Pauses/Interjections examples (short lines, "uh", "hmm", etc.):
  - "You know, I, uh, tried to measure my charisma on a Vitamedic vigor tester once the machine burst into flames."

- Exclamations and emphatic lines:
  - "Hey, get back here!"
  - "Go get em, guys!"

## Forbidden

- Modern slang and profanity should be avoided when writing for Mr. New Vegas.
- Avoid harsh, aggressive language that breaks his suave persona.
